...
- Leitura plana do `core` (sem `ST_*` no caminho de leitura — agrega counts/médias).

## Decisão / fronteira
- **F2 expõe a comparação** (verificado vs estimado). A **recalibração efetiva do score** (atualizar `model_params`/coeficientes a partir do `v_real`) é um passo do **normalizer/pipeline**, não do intel — fica como **próximo passo** (pós-medição), referenciado aqui.
